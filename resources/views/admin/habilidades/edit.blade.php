        ...
        <div class="mb-3">
            ...
            >
                @if($habilitarSelect)
                    ...
                    @foreach($tipos as $t)
                        <option value="{{ $t->id }}" @selected($valorTipoActual == $t->id)>
                            {{-- CORRECCIÓN AQUÍ --}}
                            {{ $t->nombre_habilidad }}
                        </option>
                    @endforeach
                @else
                    {{-- Si no hay tipos, mostramos el que ya tiene para no perderlo --}}
                    <option value="{{ $habilidad->tipo_habilidad_id }}" selected>
                        {{ $habilidad->tipo->nombre_habilidad ?? 'Tipo no disponible' }}
                    </option>
                @endif
            </select>
